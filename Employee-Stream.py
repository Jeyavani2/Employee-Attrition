...
def preprocess_data(df, target_column='Attrition', test_size=0.2, random_state=42):
    """
    Preprocesses the data (encoding, scaling) and splits it.
    Returns X_train, X_test, y_train, y_test, preprocessor, numerical_features, categorical_features.
    """
    df_processed = df.copy() # Work on a copy

    # Label Encode 'Attrition' target column
    le = LabelEncoder()
...
def get_trained_model():
    ...
    try:
        # Ensure this path is correct for your system and the model exists
...
def get_cleaned_data_for_eda():
    ...
    # Ensure 'Attrition' is in its original 'Yes'/'No' string format for EDA plots
    # This assumes the original CSV has 'Attrition' as 'Yes'/'No'
...
# Get feature lists for prediction form (from a dummy run of preprocess_data)
# This is needed to ensure the prediction form has all the fields the model expects
# We use df_eda here as it's the cleaned data before target encoding for X.
...
def predict_attrition_bulk(model, new_data_df):
    """
    Makes predictions for a DataFrame of new data.
    Assumes the model is a pipeline that handles preprocessing.
    """
    # The model (which should be a pipeline) will handle the preprocessing
...
if page == "Dashboard Overview":
    ...
    # Ensure 'EmployeeNumber', 'EmployeeCount', 'StandardHours', 'Over18' are not in numerical_cols if they exist
    ...
    # Get unique values for categorical features from the loaded data for selectboxes
    unique_education_fields = sorted(df_eda['EducationField'].unique())
    unique_education_levels = sorted(df_eda['Education'].unique()) # Use actual unique values from data
    ...
    with st.form("employee_prediction_form"):
        ...
        monthly_rate = st.slider("Monthly Rate ($)", 1000, 15000, 4000)
        hourly_rate = st.number_input("Hourly Rate", min_value=30, max_value=1000, value=65)
        ...
        if submitted:
            ...
            # This DataFrame will have shape (1, N) which is 2-dimensional and correct.
            input_df_for_prediction = pd.DataFrame([user_input])

            # --- Directly make prediction using the loaded model ---
            # Remove the make_single_prediction function and its call
            try:
                # model is expected to be a pipeline that handles preprocessing
